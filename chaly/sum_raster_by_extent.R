...
e <- as(extent(-14.194336,-0.878906,4.0067,10.919618 ), 'SpatialPolygons')
className <- "amphibians"
className2 <- stringr::str_to_title(className)
folderSpecies <- paste0("/media/victor/DATA/GRA2020/SEND_DATA/Geographic_spatial_data_terrestrial_tetrapods_062020_05/",className2,"_geographic_062020/")
...
folderResults <- paste0("/media/victor/DATA/GRA2020/SEND_DATA/ResultsChaly02/",className2,"/")
dir.create(folderResults, showWarnings = TRUE, recursive = T)
...
for(order in orders){
  ...
  for(rl in rlCat2){
    ...
    sumRaster(df_subGroup2$route, paste0(folderResults,"sum_",order,"_",rl,".tif"), e)
  }
}
## Generating All Map orderName
for(order in orders){
  fs2 <- list.files(path=folderResults, pattern = glob2rx(paste0("sum_",order,"_*.tif$")), full.names = TRUE)
  sumRaster(fs2, paste0(folderResults,"sum_",order,".tif"), e)
}
...
for(rl in rlCat2){
  ...
  sumRaster(fs2, paste0(folderResults,"sum_",rl,".tif"), e)
}
fs2 <- list.files(path=folderResults, pattern = glob2rx(paste0("sum_*_*.tif$")), full.names = TRUE)
sumRaster(fs2, paste0(folderResults,"sumTotal",className2,".tif"), e)
