...
df_postgres$nlevel2 <- factor(df_postgres$nlevel2, levels = c("Urban Development","Agricultural Activity",
                                                              "Over-Exploitation", "Invasion & Disease",
                                                              "Pollution", "System Modification",
                                                              "Climate Change"))
...
p<- ggplot(df2) +
  geom_bar(aes(x=as.factor(nlevel2), y = percent2, fill= className), stat = "identity", position = "dodge") +
  #scale_y_log10()+
...
dpi <- 400
ggsave(
  paste0(folderResults,folderGraph,"/FINAL3/threatTables_all_percentage_threat_totalThreat_",dpi,".png"),
  #ggplot_alternative(),
  width = 12,
  height = 5,
  dpi = dpi
