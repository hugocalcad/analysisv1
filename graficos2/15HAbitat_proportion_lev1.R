...
df2 <- df2 %>% tidyr::complete(className, nlevel1= unique(df2$nlevel1), fill = list(richness=0, threat = 0, percent = 0, percent2 = 0, percent3 = 0) )
df4 <- df2
df4$className <- paste0(df4$className, " THREAT")
...
p<- ggplot() +
  geom_bar(data = df2,aes(x=as.factor(nlevel1), y = percent, fill= className), alpha = 0.5, stat = "identity", position = "dodge") +
  geom_bar(data= df4, aes(x=as.factor(nlevel1), y = percent3, fill= className), stat = "identity", position = "dodge")+
  #scale_y_log10()+
  ...
  geom_text(data = df2,aes(x=as.factor(nlevel1), y = percent, fill= className, label = ifelse(percent == 0, "", paste0(round(percent,1),"%"))), 
            size=1.85, hjust=0.5, vjust =-0.2, position=position_dodge(width=0.9), 
            angle = 0)+
  geom_text(data = df4,aes(x=as.factor(nlevel1), y = -2, fill= className, label = ifelse(percent3 == 0, "", paste0(round(percent3,1),"%"))), 
            size=1.85, hjust=0.5, vjust =-0.2, position=position_dodge(width=0.9), 
            angle = 0)
...
dpi <- 400
ggsave(
  paste0(folderResults,folderGraph,"/FINAL4/ThreathHabitatLEvel1Proportions_",dpi,"_v1.png"),
  #ggplot_alternative(),
  width = 10,
  height = 5,
  dpi = dpi
