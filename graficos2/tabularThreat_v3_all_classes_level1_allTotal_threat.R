...
p<- ggplot() +
  geom_bar(data = df2,aes(x=as.factor(nlevel1), y = richness, fill= className), alpha = 0.5, stat = "identity", position = "dodge") +
  geom_bar(data= df4, aes(x=as.factor(nlevel1), y = threat, fill= className), stat = "identity", position = "dodge")+
  #scale_y_log10()+
  #facet_wrap(~level1, scales = "free")+
  #scale_y_log10()+
  labs(y = paste0("Total Number of Species"))+
...
ggsave(
  paste0(folderResults,folderGraph,"/threatTables_all_rich_vs_threat_v2_5_3.png"),
  #ggplot_alternative(),
  width = 10,
  height = 5,
  dpi = 300
