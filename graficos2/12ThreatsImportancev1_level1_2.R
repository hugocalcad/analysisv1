...
folderGraph <- "graphicsGeneral/FINAL3"
...
df_table_level1_2 <- df_postgres %>%
  filter(level1 == 5) %>%
  filter(scope == "Whole (>90)") %>%
  filter(severity == "Very Rapid Declines")

df_table_level1_2 <- df_postgre3 %>%
  filter(rlCodes %in% c("CR", "EN", "VU")) %>%
  group_by(className, nlevel2S2) %>%
  summarize(n=n()) %>%
  tidyr::spread(key = className, value= n)

write.csv(df_table_level1_2, paste0(folderResults, folderGraph, "/threat_level1_2_all_v2.csv"))

df_table_level1_2_reptiles <- df_postgre3 %>%
  filter(className == "REPTILIA") %>%
  filter(rlCodes %in% c("CR", "EN", "VU")) %>%
  group_by(orderName2, nlevel2S2) %>%
  summarize(n=n()) %>%
  tidyr::spread(key = orderName2, value= n)

write.csv(df_table_level1_2_reptiles, paste0(folderResults, folderGraph, "/threat_level1_2_reptiles_v2.csv"))
